...
function Header() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu - Separate from header for better z-index control */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-black/50 z-[100] md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
          <div 
            className="fixed top-0 right-0 h-full w-64 bg-gradient-to-b from-indigo-600 to-purple-700 shadow-xl"
                ...
                {authStatus ? (
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-white/70 truncate max-w-[150px]">{userData?.email}</span>
                    </div>
                  </div>
                ) : (
